...
echo -e "${CYN}      The Lobster Way · v2.0${NC}"
echo "  ─────────────────────────────────"
echo ""

# ── 1. 检查 swiftc ──────────────────────────
if ! command -v swiftc &>/dev/null; then
    echo -e "  ${YLW}⚠${NC}  未找到 swiftc，正在触发 Xcode CLI Tools 安装..."
...
echo -e "  ${GRN}✓${NC} swiftc $(swiftc --version 2>&1 | head -1 | grep -o 'Swift [0-9.]*' || echo 'found')"
...
# ── 3. 预览匹配结果 ──────────────────────────
c=0; cy=0; z=0
for w in social friend fun happy creative energetic art music humor laugh; do [[ "$SOUL" == *"$w"* ]] && ((c+=2)) || true; done
for w in code hack terminal engineer debug algorithm tech script api git data optimize; do [[ "$SOUL" == *"$w"* ]] && ((cy+=2)) || true; done
for w in calm peace balance mindful wisdom deep slow patient meditate philosophy reflect; do [[ "$SOUL" == *"$w"* ]] && ((z+=2)) || true; done
...
# ── 4. 创建目录 ──────────────────────────────
mkdir -p "$BIN_DIR"
echo -e "  ${GRN}✓${NC} 创建 ~/.openclaw/pets/"

# ── 5. 写入 Swift 源码 ───────────────────────
echo -e "  ${GRN}✓${NC} 生成 molty.swift..."
...
echo -e "  ${GRN}✓${NC} Swift 源码已写入"

# ── 6. 编译 ──────────────────────────────────
echo ""
echo -e "  ${CYN}⚙  正在编译（首次约需 15 秒）...${NC}"
if swiftc -framework WebKit -O -o "$BIN_DIR/molty" "$SWIFT_SRC" 2>&1 | grep -i error; then
    echo -e "  ${RED}✗  编译失败，请检查以上错误${NC}"
    exit 1
fi
chmod +x "$BIN_DIR/molty"
echo -e "  ${GRN}✓${NC} 编译成功 → Molty.app"
...
echo ""
echo "  · 桌面萌宠完全透明、可穿透点击，不影响任何操作"
echo "  · 开机自动启动，soul.md 更新后 10 分钟内自动换风格"
echo "  · 卸载：launchctl unload $AGENT_PLIST && rm -rf $APP_DIR"
echo "  · 日志：$PETS_DIR/molty.log"
echo ""
